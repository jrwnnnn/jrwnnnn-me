document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.glitch').forEach(el => {
        ...
        const layers = [0, 1].map(() => {
            const l = wrapper.cloneNode(true);
            l.style.cssText = "position:absolute; top:0; left:0; width:100%; height:100%; opacity:0; pointer-events:none; z-index:10; mix-blend-mode:hard-light;";
            l.querySelectorAll('[id]').forEach(n => n.id = '');
            el.appendChild(l);
            return l;
        });
        ...
        const loop = () => {
            ...
            if (f % 10 === 0) {
                ...
                layers.forEach(l => {
                    if (!active) { l.style.opacity = '0'; return; }
